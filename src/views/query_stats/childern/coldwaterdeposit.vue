          ...
          <div class="padding_table">
            <el-table :data="TableData.records" v-loading="loading">
              ...
              <el-table-column label="充值总额(元)">
                <template slot-scope="scope">{{ scope.row.rechargeSum || 0 }}</template>
              </el-table-column>
              <el-table-column label="消费总额(元)">
                <template slot-scope="scope">{{ scope.row.consumerSum || 0 }}</template>
              </el-table-column>
              <el-table-column label="退款金额(元)">
                <template slot-scope="scope">{{ scope.row.refundSum || 0 }}</template>
              </el-table-column>
              <el-table-column label="退款中总额(元)">
                <template slot-scope="scope">{{ scope.row.refundingSum || 0 }}</template>
              </el-table-column>
...
export default {
  ...
  methods: {
    jumprouter(row) {
      let val = { label: "冷水信息", name: "cold_water", path: "/query_stats/cold_water" };
      let result = this.$store.state.tab.tabsList.findIndex(
        (item) => item.name === "cold_water"
      );
    ...
    jz_confirm() {
      ...
      }else if(/^\d+$/.test(this.jz_money)==false || this.jz_money<=0){
        this.$message.warning('纠正金额必须为数字或者大于0!');
        this.jz_money=null;
        ...
        this.$axios
          ...
          .then((res) => {
            ...
            if (res.data.code == 200) {
              this.TableSearch();
              return this.$message.success("纠正成功");
            } else {
