    ...
    <div class="right fr">
      ...
      <div class="table_data">
        ...
        <div class="consumption" v-if="rightheadertab == 1">
          <div class="top_title clearfix">
            <h3 class="fl">
              <div>
                <span>总用水量(吨):</span>
                {{ ConsumptionTotal.totalUseAmount || 0 }}
              </div>
              <div>
                <span>消费总额(元):</span>
                {{ ConsumptionTotal.totalUseMoney || 0 }}
              </div>
            </h3>
            <el-button type="primary" class="jshidden fr">
              <i class="el-icon-folder-add el-icon--left"></i>
        ...
        <div class="refund" v-if="rightheadertab == 2">
          <ul>
            <li
              :class="refundListnumber == index ? 'active' : ''"
              @click="
                refundListnumber = index;
                search();
                CurrentPage = 1;
              "
          ...
          <div class="top_title clearfix">
            <h3 class="fl">
              <div v-if="refundListnumber == 0">
                <span>待处理总额(元):</span>{{ RefundTotal.totalMoney }}
              </div>
              <div v-else><span>退款总额(元):</span>{{ RefundTotal.totalMoney }}</div>
            </h3>
            <el-button type="primary" class="jshidden fr">
              <i class="el-icon-folder-add el-icon--left"></i>
              导出列表
              ...
              <template slot-scope="scope">
                {{
                  scope.row.roleType == 1
                    ...
                    ? "维修工"
                    : scope.row.roleType == 7
                    ? "网络信息中心"
                    : scope.row.roleType == 0
                    ? "默认用户角色"
                    : scope.row.roleType == 8
                    ? "财务主管"
                    : scope.row.roleType == 10
                    ? "演示"
                    : scope.row.roleType == 11
                    ? "公有云租户"
                    : scope.row.roleType == 13
                    ? "资源管理员"
                    : scope.row.roleType == 14
                    ? "客户商户"
                    : scope.row.roleType == 15
                    ? "财务出纳"
                    : "-"
                }}
              </template>
            ...
            <el-table-column label="操作">
              <template slot-scope="scope">
                ...
                <span
                  ...
                  >审批</span
                >
                <el-dropdown class="fontblue" v-if="scope.row.refundStatus == 1">
        ...
        <div class="clear" v-if="rightheadertab == 3">
          <div class="top_title butonnhover clearfix">
            ...
            <el-button type="primary" class="fr jshidden">
              <i class="el-icon-folder-add el-icon--left"></i>
              导出列表
            </el-button>
          </div>
          ...
          <table v-loading="loading">
            ...
            >
              <tr v-for="(item, index) in StatisticsTableData.records" :key="index">
                ...
                <td v-if="item.layer == 3" @click="StatisticsDIalogClick(item)">
                  <span class="fontblue">查看详情</span>
                </td>
              </tr>
            </template>
            <tr v-else>
...
export default {
  ...
  data() {
    return {
      ...
      ConsumptionTotal: {}, //消费总和
      ConsumptionDailog: false, //消费弹窗
  ...
  created() {
    ...
    //首页本月总收入
    if (this.$route.query.coldincome) {
      this.timecheck = 1;
      this.sjfw_value = "0";
  ...
  methods: {
    StatisticsDialogTimeChange() {
      if (this.StatisticsDialogTime == null) {
        this.StatisticsDialogCurrentPage = 1;
      }
    ...
    // 用量统计Dialogtable
    AxiosStatisticsDialogTableData() {
      this.$axios
        .post("/iot/cold-water/getColdUseDetailsList", {
          layer: this.StatisticsDialogLayer,
          areaId: this.StatisticsDailogAreaId,
          endDate:
            this.StatisticsDialogTime == null ? null : this.StatisticsDialogTime[1],
          startDate:
            this.StatisticsDialogTime == null ? null : this.StatisticsDialogTime[0],
    ...
    AxiosStatisticsColdUseTotal(startDate, endDate) {
      this.layer =
        this.layer !== null && this.layer !== "" ? parseInt(this.layer) : this.layer;
      this.$axios
    ...
    AxiosRefundMoney(startDate, endDate) {
      this.$axios
        .post("/pay/refund/getColdTotal", {
          ...
          startDate,
          endDate,
          haveDeal: this.refundListnumber + 1,
        })
        .then((res) => {
    ...
    AxiosConsumptionTotal(startDate, endDate) {
      this.$axios
        ...
        .then((res) => {
          this.ConsumptionTotal = res.data;
        });
    },
    // 充值table
...
.refund {
  ...
  ul {
    ...
    li {
      ...
      &.active {
        color: #2b9cff;
        border-bottom: 2px solid #2b9cff;
      }
      &:hover {
        color: #2b9cff;
      }
