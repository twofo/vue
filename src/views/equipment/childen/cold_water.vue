    ...
    <div class="filtrate">
      ...
      <div class="filtrate-table">
        <div class="filtrate-table-button clearfix">
          ...
          <button class="fr pink" v-preventReClick="2000" @click="AlldelDialog()">
            删除
          </button>
        ...
        >
          ...
          <el-table-column label="费控开关">
            <template slot-scope="scope" v-if="scope.row.deviceCache">
              <i
                :class="
                  scope.row.deviceCache.attributes.isNormallyOpenMode == 2
                    ? 'green'
                    : scope.row.deviceCache.attributes.isNormallyOpenMode == 0
                    ? 'gray'
                    : ''
                "
              ></i>
    ...
    >
      <div class="info" v-if="essentialinformation">
        ...
        <div class="button">
          ...
          <button v-preventReClick="2000" class="jshidden">阀门开关</button>
        </div>
        <ul>
          <li>
            <label>设备编号:</label
          ...
          <li>
            ...
            <span
              ...
              "
              >{{
                essentialinformation.deviceCache.status.enable == false ? "禁用" : "启用"
              }}</span
            >
            <span v-else>-</span>
          </li>
          ...
          <li>
            <label>阀门开关:</label>
            <span v-if="essentialinformation.deviceCache">{{
              essentialinformation.deviceCache.status.isOpen == 0 ? "关闭" : "开启"
            }}</span>
            <span v-else>-</span>
          </li>
          <li>
      ...
      <div class="record-list" v-if="essentialinformation">
        ...
        <div class="record-list-tab">
          <div>
            <span
              ...
              v-for="(item, index) in essentialinformation.gatewayType == 6
                ? record_list_tabtwo
                : record_list_tab"
              :key="index"
              @click="
        ...
        <table class="Listannal" v-loading="drawerloading">
          <tr v-if="essentialinformation.gatewayType !== 6 && record_list_tab_num == 0">
            <td>操作渠道</td>
            <td>操作类型</td>
            <td>账号</td>
            <td>姓名</td>
          ...
          <tr v-if="essentialinformation.gatewayType !== 6 && record_list_tab_num == 1">
            <td>卡号</td>
            <td>用水方式</td>
            <td>用水量(升)</td>
            <td>时间</td>
          </tr>
          <tr v-if="essentialinformation.gatewayType !== 6 && record_list_tab_num == 2">
            <td>区域信息</td>
            <td>告警记录</td>
            <td>时间</td>
          </tr>
          ...
          <tr v-if="essentialinformation.gatewayType !== 6 && record_list_tab_num == 3">
            <td>设备状态</td>
            <td>上线/离线时间</td>
          </tr>
          <template
          ...
          >
            <tr v-for="(item, index) in OperateListtable.records" :key="index">
              <!-- 操作记录 -->
              <td v-if="essentialinformation.gatewayType !== 6 && record_list_tab_num == 0">
                {{ item.operateChannel || "-" }}
              </td>
              <td v-if="essentialinformation.gatewayType !== 6 && record_list_tab_num == 0">
                {{ item.operateAction || "-" }}
              ...
              <td v-if="essentialinformation.gatewayType !== 6 && record_list_tab_num == 0">{{ item.useHotWater }}</td>
              <td v-if="essentialinformation.gatewayType !== 6 && record_list_tab_num == 0">{{ item.money }}</td>
              <td v-if="essentialinformation.gatewayType !== 6 && record_list_tab_num == 0">
              ...
              <!-- 异常用水记录 -->
              <td
                v-if="essentialinformation.gatewayType !== 6 && record_list_tab_num == 1"
              >
                {{ item.cardNo || "-" }}
              </td>
              <td
              ...
              <td
                v-if="essentialinformation.gatewayType == 6 && record_list_tab_num == 0"
              >
                {{ item.cardNo || "-" }}
              </td>
              <td
              ...
              <!-- 告警记录 -->
              <td v-if="essentialinformation.gatewayType == 6 && record_list_tab_num == 1">{{ item.address || "-" }}</td>
              <td v-if="essentialinformation.gatewayType == 6 && record_list_tab_num == 1">{{ item.alarmInfo || "-" }}</td>
              <td v-if="essentialinformation.gatewayType == 6 && record_list_tab_num == 1">{{ item.alarmTime || "-" }}</td>
              <td v-if="essentialinformation.gatewayType !== 6 && record_list_tab_num == 2">{{ item.address || "-" }}</td>
              <td v-if="essentialinformation.gatewayType !== 6 && record_list_tab_num == 2">{{ item.alarmInfo || "-" }}</td>
              <td v-if="essentialinformation.gatewayType !== 6 && record_list_tab_num == 2">{{ item.alarmTime || "-" }}</td>
              <!-- 上下线 -->
              <td
                v-if="essentialinformation.gatewayType !== 6 && record_list_tab_num == 3"
              >
                {{ item.deviceStatus || "-" }}
              </td>
              <td
...
export default {
  data() {
    return {
      ...
      record_list_tab: ["操作记录", "异常用水记录", "告警记录", "设备上下线日志"],
      record_list_tabtwo: ["异常用水记录", "告警记录"],
  ...
  methods: {
    ...
    // 操作记录table查询
    OperateListtablesearch() {
      if (this.essentialinformation.gatewayType !== 6) {
        if (this.record_list_tab_num == 0) {
          ...
          //异常用水
          this.$axios
            .post("/iot/device/getErrorUseWaterDetail", {
              current: this.ListannalCurrent,
              endDate: this.dateend,
              startDate: this.datestart,
              size: 5,
    ...
    getInfoTableSearch() {
      this.$axios
        .post("/iot/device/coldWaterDetail", {
    ...
    getdetailstotal() {
      this.$axios
        .post("/iot/device/coldWaterDetailTotal", {
          deviceCode: this.essentialinformation.deviceCode,
          startDate: this.PickerstartTime,
          endDate: this.PickerendTime,
        })
